    ...
    MongoClient.connect('mongodb://localhost', (err, client) => {
        ...
        log.find().sort({ "log_created": -1 }).limit(10).toArray((err, recent) => {
            for (index = 0; index < recent.length; index++) {
                ...
                let endTime = new Date(moment(recent[index]['ab_res_creation_time']).tz('America/New_York'));
                recent[index]['ab_res_creation_time'] = endTime;//.format('YYYY-MM-DD HH:mm:ss').toString();
            }
